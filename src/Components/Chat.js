...
const Chat = ({ addUserChat, userChat, setuserChat, sendUserSessionData, chatHistory, setChatHistory, isChatLoading = false, onOpenImagesList, imagesAvailable = false, onGoToScenes, scenesMode = false, initialScenes = null, onBackToChat, enablePresenterOptions = false, isSwitchingVideoType = false, loadingVideoType = null, onScriptChange = null }) => {
  ...
  const [hasSessionImages, setHasSessionImages] = useState(false);
                    ...
                    <div className="flex gap-2 border-b border-gray-200 mb-4 overflow-x-auto scrollbar-hide">
                      {rows.map((r, i) => (
                        <button
                          ...
                          className={`px-4 py-2 text-sm font-medium whitespace-nowrap transition-colors ${activeSceneTab === i
  ...
  // Check if session_data.images is not empty
  useEffect(() => {
    const checkSessionImages = async () => {
      try {
        const token = localStorage.getItem('token');
        const sessionId = localStorage.getItem('session_id');
        if (!token || !sessionId) {
          setHasSessionImages(false);
          return;
        }
        ...
        if (response.ok && data) {
          ...
          setHasSessionImages(images.length > 0);
        } else {
          setHasSessionImages(false);
        }
      } catch (error) {
        console.error('Error checking session images:', error);
        setHasSessionImages(false);
      }
    };
    checkSessionImages();
    // Also listen for session updates
    const interval = setInterval(checkSessionImages, 5000); // Check every 5 seconds
    return () => clearInterval(interval);
  }, []);
    ...
    <div className='bg-white h-[79vh] flex justify-between flex-col rounded-lg mt-3 p-6 relative'>
      ...
      {isSwitchingVideoType && loadingVideoType && (
        <Loader
          ...
          progress={switchingModelProgress > 0 ? switchingModelProgress : null}
        />
      )}
      {/* Add Scene Modal */}
      {showAddSceneModal && (
                  ...
                  <div className="flex gap-3 flex-wrap">
                    {['Avatar Based', 'Infographic', 'Financial'].map((type) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${newSceneVideoType === type
                          ...
                          {avatarPresenterOptions.map((preset, idx) => {
                            ...
                            return (
                              <button
                                ...
                                className={`w-full text-left border rounded-lg px-3 py-3 transition-all ${selected
            ...
            <div className="px-6 pt-4 pb-6 flex justify-end gap-2 flex-shrink-0 border-t border-gray-200">
              ...
              {addSceneStep === 1 && (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium text-white ${isLoadingNewScenePresenter || isSuggestingScenes || (newSceneVideoType === 'Financial' && !newSceneChartType)
              ...
              {requiresAvatarPresenterSelection && addSceneStep === 2 && (
                <button
                  ...
                  className={`px-4 py-2 rounded-lg text-sm font-medium text-white ${isLoadingNewScenePresenter || isSuggestingScenes || !newScenePresenterPresetId
                    ? 'bg-[#9aa0d0] cursor-not-allowed'
                    : 'bg-[#13008B] hover:bg-blue-800'
                    }`}
                >
                              ...
                              {switchPresenterOptions.map((po) => {
                                ...
                                return (
                                  <button
                                    ...
                                    className={`relative flex flex-col overflow-hidden rounded-lg border transition-shadow ${isSelected
              ...
              <div className="relative flex items-center gap-2">
                {!isEditingScene && hasOrderChanged && (
                  <button
                    ...
                    disabled={isSavingReorder}
                    className={`inline-flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium ${isSavingReorder ? 'bg-green-400 text-white cursor-not-allowed' : 'bg-green-600 text-white hover:bg-green-700'
                      }`}
                ...
                {hasSessionImages && (
                  <button
                    onClick={() => {
                      if (typeof onOpenImagesList === 'function') {
                        onOpenImagesList(imagesJobId || '');
                      } else {
                        setShowImagesOverlay(true);
                      }
                    }}
                    className="inline-flex items-center gap-1 px-3 py-2 rounded-lg text-sm font-medium text-black border-[1px] border-black bg-white hover:bg-[#eee]"
                      ...
                      return (
                        ...
                        >
                          ...
                          <button
                            ...
                            className={`px-3 py-1.5 rounded-full text-sm font-medium transition-colors ${currentSceneIndex === index
                              ? 'bg-[#13008B] text-white'
                  ...
                  {(Array.isArray(scriptRows) ? scriptRows : []).map((r, i) => {
                    ...
                    return (
                      <div
                        ...
                        className={`grid grid-cols-12 gap-[3px] text-center text-[10px] text-gray-800 transition-all duration-200 ${isDragging ? 'script-row-dragging' : ''
                  ...
                  <div className="space-y-6">
                    ...
                    {(() => {
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        {hasChartTypeChanged && (
                                          <button
                                            type="button"
                                            disabled={isRegeneratingChart}
                                            className={`px-3 py-1.5 rounded-lg text-sm ${isRegeneratingChart
                      ...
                      if (isVEO3) {
                        ...
                        const sceneForBackgroundToggle = Array.isArray(scriptRows) ? scriptRows[currentSceneIndex] : null;
                        const modelUpperForBackgroundToggle = String(sceneForBackgroundToggle?.model || sceneForBackgroundToggle?.mode || '').toUpperCase();
                        const isVeo3SceneForBackground = modelUpperForBackgroundToggle === 'VEO3';
                        const hasBackgroundImage = Array.isArray(sceneForBackgroundToggle?.background_image) && sceneForBackgroundToggle.background_image.length > 0;
                        let backgroundGenVal = typeof sceneForBackgroundToggle?.gen_image === 'boolean' ? sceneForBackgroundToggle.gen_image : undefined;
                        if (typeof backgroundGenVal !== 'boolean') {
                          backgroundGenVal = !hasBackgroundImage;
                        }
                        const backgroundToggleDisabled = isSavingSceneData;
                        const backgroundToggleBase = `inline-flex items-center gap-3 px-2 py-1 rounded-full transition-colors ${backgroundGenVal ? 'bg-green-100' : 'bg-gray-100'}`;
                        const backgroundToggleCls = backgroundToggleBase;
                        const onBackgroundToggle = async () => {
                          if (backgroundToggleDisabled) return;
                          const next = !backgroundGenVal;
                          try {
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                              {templateFields.length > 0 ? (
                                templateFields.map(([key, value]) => {
                                  ...
                                  const isBackgroundField = keyLower.includes('background');
                                  const showHeaderToggle = isBackgroundField && isVeo3SceneForBackground && !hasBackgroundImage;
                                  return (
                                    <div
                                      key={key}
                                      className="border border-gray-200 rounded-xl bg-white p-4 space-y-2 shadow-sm relative"
                                    >
                                      <div className="flex items-center justify-between">
                                        ...
                                        {showHeaderToggle && (
                                          <button
                                            ...
                                            className={backgroundToggleCls}
                                            title="Toggle Generate Image"
                                            disabled={backgroundToggleDisabled}
                                          >
                                            <span className="text-sm text-gray-800">{backgroundGenVal ? 'True' : 'False'}</span>
                                            <span className={`relative inline-flex w-10 h-5 rounded-full ${backgroundGenVal ? 'bg-green-500' : 'bg-gray-400'}`}>
                                              <span className={`absolute top-0.5 ${backgroundGenVal ? 'right-0.5' : 'left-0.5'} w-4 h-4 bg-white rounded-full transition-all`} />
                                            </span>
                                          </button>
                                        )}
                                      </div>
                                      ...
                                      {isBackgroundField && isVeo3SceneForBackground && hasBackgroundImage && (
                                        <div className="absolute inset-0 rounded-xl bg-white/80 flex items-center justify-between">
                                          <div className="m-2 px-2.5 py-1 rounded-full text-[15px] font-semibold tracking-wide text-black">
                            ...
                            {isCurrentlyEditing ? (
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  type="button"
                                  disabled={isUpdatingText}
                                  className={`px-3 py-1.5 rounded-lg text-sm ${isUpdatingText
                                    ? 'bg-blue-400 text-white cursor-not-allowed'
                                    : 'bg-[#13008B] text-white hover:bg-blue-800'
                                    }`}
                                  onClick={saveEditing}
                              ...
                              <div className="flex flex-wrap gap-3">
                                {showAvatarOption && (
                                  <button
                                    ...
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${(sceneModelUpper === 'VEO3' || sceneModelUpper === 'ANCHOR')
                                      ? 'bg-[#13008B] text-white'
                                ...
                                {showInfographicOption && (
                                  <button
                                    ...
                                    className={`px-4 py-2 rounded-full text-sm font-medium transition-colors ${sceneModelUpper === 'SORA'
                                      ? 'bg-[#13008B] text-white'
                                      : 'bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed'
                                      }`}
                                  >
                                ...
                                {showFinancialOption && (
                                  <button
                                    ...
                                    className={`px-4 py-2 rounded-full text-sm font-medium ${sceneModelUpper === 'PLOTLY' ? 'bg-[#13008B] text-white' : 'bg-gray-100 text-gray-400 border border-gray-200 cursor-not-allowed'
                                      }`}
                    ...
                    {(() => {
                      ...
                      const showToggleInBackgroundSection = !isSummary && modelUpper !== 'VEO3';
                    ...
                    {(() => {
                      ...
                      return (
                        <div className="mt-4">
                          ...
                          {advancedOptionsOpen && (
                            <div className={`mt-3 rounded-lg border border-gray-200 bg-gray-50 transition-all duration-300 ${showRichTextEditor ? 'flex flex-col md:flex-row gap-4' : ''
                              }`}>
                              <div className={`space-y-4 px-4 py-4 transition-all duration-300 ${showRichTextEditor ? 'w-full md:w-1/2' : 'w-full'
                                }`}>
                                <div className="flex items-center justify-between">
                                    ...
                                    {isEditingAdvancedStyles ? (
                                      <>
                                        <button
                                          ...
                                          className={`inline-flex items-center gap-1 rounded-md px-3 py-1.5 text-xs font-semibold text-white ${isSavingAdvancedStyles ? 'bg-green-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'
                                            }`}
                                        >
                                    ...
                                    <>
                                      <div>
                                        ...
                                        <div className="flex flex-wrap items-center gap-3">
                                          <button
                                            ...
                                            disabled={!canEditStyles}
                                            className={`h-9 w-9 rounded-full border text-lg font-semibold ${canEditStyles ? 'border-gray-300 bg-white text-gray-700 hover:bg-gray-50' : 'border-gray-200 bg-gray-100 text-gray-400 cursor-not-allowed'
                                              }`}
                                          >
                                            −
                                          </button>
                                          ...
                                          <input
                                            ...
                                            disabled={!canEditStyles}
                                            className={`w-20 rounded-md border px-2 py-1 text-sm ${canEditStyles ? 'border-gray-300' : 'border-gray-200 bg-gray-100 text-gray-500 cursor-not-allowed'
                                              }`}
                                          />
                                      ...
                                      <div>
                                        ...
                                        <select
                                          ...
                                          className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-[#13008B] focus:border-transparent ${canEditStyles ? 'border-gray-300' : 'border-gray-200 bg-white text-gray-500 cursor-not-allowed'
                                            }`}
                                          disabled={!canEditStyles}
                                        >
                                      ...
                                      <div className="grid grid-cols-5 gap-2">
                                        {['rectangle', 'circle', 'triangle', 'line', 'arrow', 'star'].map((shape) => (
                                          <button
                                            ...
                                            className={`h-16 rounded-lg border-2 transition-all ${selectedShape === shape
                                          ...
                                          {isEditingAnchorPrompt ? (
                                            <>
                                              <button
                                                ...
                                                className={`inline-flex items-center gap-1 rounded-md px-3 py-1.5 text-xs font-semibold text-white ${isSavingAnchorPrompt ? 'bg-green-400 cursor-not-allowed' : 'bg-green-600 hover:bg-green-700'
                                                  }`}
                                              >
                  ...
                  <>
                    ...
                    {!isEditingScene && hasOrderChanged && (
                      <button
                        onClick={() => setShowSaveConfirm(true)}
                        disabled={isSavingReorder}
                        className={`px-4 py-2 rounded-lg text-sm transition-colors text-white ${isSavingReorder
      ...
      {isApplyingChartType && (
        <Loader
          ...
          progress={chartTypeProgress > 0 ? chartTypeProgress : null}
        />
      )}
            ...
            <div className="p-4 overflow-y-auto">
              {(isAssetsLoading || (['preset_templates', 'uploaded_templates'].includes(assetsTab) && isLoadingTemplates) || (assetsTab === 'generated_images' && isLoadingGeneratedImages)) ? (
                ...
                if (assetsTab === 'preset_templates' || assetsTab === 'uploaded_templates') {
                  ...
                  const allEntries = Object.entries(generatedImages);
                  let filteredEntries = allEntries;
                  if (normalizedTargetAspect && normalizedTargetAspect !== 'Unspecified') {
                    filteredEntries = allEntries.filter(([aspectRatio]) =>
                      matchesAspectValue(aspectRatio, normalizedTargetAspect)
                    );
                  }
                  generatedImageGroups = buildGroups(filteredEntries);
                  if (generatedImageGroups.length === 0 && (normalizedTargetAspect && normalizedTargetAspect !== 'Unspecified')) {
                    list = [];
                    generatedImageGroups = buildGroups(allEntries);
                  }
                }
                ...
                if (assetsTab === 'generated_images') {
                  const message = generatedImageGroups.length === 0 ? emptyMessage : '';
                  return (
                    <>
              ...
              <div className="mt-4 flex items-center justify-end gap-2 border-t pt-3">
                <button
                  ...
                  className={`px-3 py-2 rounded-lg text-sm border ${selectedTemplateUrls.length === 0
      ...
      {isEnhancing && (
        <Loader
          ...
          progress={enhancingProgress > 0 ? enhancingProgress : null}
        />
      )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-end pt-1">
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-medium text-white ${isRegenerating || isLoadingRegenPresenter || isSuggestingRegen || !regenChartType
                        ? 'bg-[#9aa0d0] cursor-not-allowed'
                        : 'bg-[#13008B] hover:bg-blue-800'
                        }`}
                    >
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex justify-end gap-2">
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-medium ${isRegenerating
                    ...
                    <button
                      ...
                      className={`px-4 py-2 rounded-lg text-sm font-medium text-white ${isRegenerating || !regenPresenterPresetId
                        ? 'bg-[#9aa0d0] cursor-not-allowed'
                        : 'bg-[#13008B] hover:bg-blue-800'
                        }`}
                    >
      ...
      {showShortGenPopup && (
        <Loader
          ...
          description="Redirecting to Images in a moment."
        />
      )}
      ...
      {(isUpdatingText || isSavingReorder || isUploadingAvatar || isUploadingSceneImages) && (
        <Loader
          ...
          progress={updatingProgress > 0 ? updatingProgress : null}
        />
      )}
                    ...
                    {[0, 1, 2, 3].map((i) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div
                            className={`w-full px-4 py-3 rounded-lg border animate-pulse ${isUser ? 'bg-[#EDEBFF] border-[#D9D6FF]' : 'bg-white border-gray-200'
                              }`}
                          >
                    ...
                    {chatHistory.map((message) => (
                      ...
                      >
                        ...
                        <div className={(message.type === 'ai' || message.type === 'thinking') ? 'flex flex-col items-start gap-2 w-full' : 'flex items-end gap-2 w-full'}>
                          <div
                            className={`${(message.type === 'ai' || message.type === 'thinking') ? 'w-full' : 'ml-auto max-w-[70%]'} px-4 py-3 rounded-2xl ${message.type === 'user'
                              ? 'chat-user-bubble'
                              : message.type === 'error'
                                  ...
                                  return (
                                    hasSessionImages && (
                                      <button
                                        ...
                                        <span className={`text-sm font-medium`}>Go to Storyboard</span>
                                      </button>
                                    )
                                  );
                                } else {
                  ...
                  <div className="flex-1 relative">
                    ...
                    <div className="absolute right-2 top-1/2 -translate-y-1/2 flex gap-1">
                      ...
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-colors ${inputMessage.trim() && !isLoading
