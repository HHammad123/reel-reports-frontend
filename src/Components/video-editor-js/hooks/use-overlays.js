...
export const useOverlays = (initialOverlays, waitForAutosave) => {
     ...
     */
    const addOverlay = useCallback((newOverlay) => {
        let newId;
        setOverlays((prevOverlays) => {
            newId =
                prevOverlays.length > 0
                    ...
                    : 0;
            const overlayWithNewId = { ...newOverlay, id: newId };
    ...
    const deleteOverlaysByRow = useCallback((row) => {
        setOverlays((prevOverlays) => prevOverlays.filter((overlay) => overlay.row !== row));
        setSelectedOverlayId(null);
    }, []);
    /**
    ...
    const duplicateOverlay = useCallback((id) => {
        setOverlays((prevOverlays) => {
            ...
            const overlaysInRow = prevOverlays.filter((o) => o.row === overlayToDuplicate.row && o.id !== id);
            ...
            while (hasOverlap) {
                hasOverlap = overlaysInRow.some((existingOverlay) => {
                    ...
                    return ((newFrom >= existingOverlay.from && newFrom < existingEnd) ||
                ...
                if (hasOverlap) {
                    ...
                    const lastOverlay = [...overlaysInRow].sort((a, b) => b.from + b.durationInFrames - (a.from + a.durationInFrames))[0];
                    newFrom = lastOverlay
     ...
     * splitOverlay(1, 100)
     */
    const splitOverlay = useCallback((id, splitFrame) => {
        ...
        setOverlays((prevOverlays) => {
            ...
            if (splitFrame <= overlayToSplit.from ||
            ...
            const secondHalfStartTime = calculateSecondHalfStartTime(overlayToSplit, firstPartDuration);
            // Create split overlays
            const [firstHalf, secondHalf] = createSplitOverlays(overlayToSplit, newId, splitFrame, firstPartDuration, secondPartDuration, secondHalfStartTime);
     ...
     */
    const handleOverlayChange = useCallback((updatedOverlay) => {
 ...
 */
const calculateSecondHalfStartTime = (overlay, firstPartDurationInFrames) => {
 ...
 */
const createSplitOverlays = (original, newId, splitFrame, firstPartDuration, secondPartDuration, secondHalfStartTime) => {
    const fps = 30;
    const msPerFrame = 1000 / fps;
    ...
    const splitWaveformData = (originalWaveformData, firstDuration, secondDuration, totalDuration) => {
        var _a;
        if (!((_a = originalWaveformData === null || originalWaveformData === void 0 ? void 0 : originalWaveformData.peaks) === null || _a === void 0 ? void 0 : _a.length)) {
            return [null, null];
        }
    ...
    if (original.type === OverlayType.SOUND && original.waveformData) {
        ...
        [firstHalfWaveformData, secondHalfWaveformData] = splitWaveformData(original.waveformData, firstDurationSeconds, secondDurationSeconds, totalDurationSeconds);
    }
    const firstHalf = {
        ...original,
        durationInFrames: firstPartDuration,
