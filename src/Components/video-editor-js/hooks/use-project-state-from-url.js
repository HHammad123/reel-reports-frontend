 ...
 */
export function useProjectStateFromUrl(paramName = "projectId", projectId = "TestComponent") {
    const fallbackOverlays = DEFAULT_OVERLAYS;
    const [overlays, setOverlays] = useState(fallbackOverlays);
    const [aspectRatio, setAspectRatio] = useState(null);
    const [backgroundColor, setBackgroundColor] = useState(null);
    const [isLoading, setIsLoading] = useState(true);
    const [showModal, setShowModal] = useState(false);
    // Store project data when we need user confirmation
    const pendingProjectDataRef = useRef(null);
    ...
    useEffect(() => {
        ...
        const isDisabled = process.env.NEXT_PUBLIC_DISABLE_PROJECT_LOADING === "true";
        if (isDisabled) {
            console.log("[useProjectStateFromUrl] Project loading is disabled via environment variable");
        ...
        const loadProjectState = async () => {
            ...
            try {
                ...
                if (!paramValue) {
                    console.log(`[useProjectStateFromUrl] No '${paramName}' parameter found, using fallback overlays`);
                    setOverlays(fallbackOverlaysRef.current);
                    setIsLoading(false);
                    return;
                }
                console.log(`[useProjectStateFromUrl] Fetching project state '${paramValue}' from API...`);
                // Fetch project state from API
                const response = await fetch(`/api/projects?id=${paramValue}`);
                if (!response.ok) {
                    console.warn(`[useProjectStateFromUrl] API returned ${response.status} for parameter '${paramValue}'`);
                    setOverlays(fallbackOverlaysRef.current);
                    setIsLoading(false);
                    return;
                }
                ...
                if (!projectData.overlays || !Array.isArray(projectData.overlays)) {
                    console.error(`[useProjectStateFromUrl] Project '${paramValue}' does not have a valid overlays array`, projectData);
                    setOverlays(fallbackOverlaysRef.current);
                    setIsLoading(false);
                    return;
                }
                console.log(`[useProjectStateFromUrl] Successfully fetched ${projectData.overlays.length} overlays from project '${paramValue}'`);
                // Extract aspect ratio if provided (API uses snake_case, convert to camelCase)
                const projectAspectRatio = projectData.aspect_ratio || projectData.aspectRatio;
                if (projectAspectRatio) {
                }
                // Extract background color if provided (API uses snake_case, convert to camelCase)
                const projectBackgroundColor = (_a = projectData.background_color) !== null && _a !== void 0 ? _a : projectData.backgroundColor;
