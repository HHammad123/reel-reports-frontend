export const useTimelinePositioning = () => {
     ...
     */
    const findNextAvailablePosition = (existingOverlays, visibleRows, totalDuration) => {
        // If no overlays exist, start at the beginning
        if (existingOverlays.length === 0) {
            return { from: 0, row: 0 };
        }
        const sortedOverlays = [...existingOverlays].sort((a, b) => a.from - b.from);
        ...
        for (let row = 0; row < visibleRows; row++) {
            const overlaysInRow = sortedOverlays.filter((overlay) => overlay.row === row);
            ...
            for (const overlay of overlaysInRow) {
                if (overlay.from - lastEndTime >= 1) {
                    ...
                    const isOverlapping = sortedOverlays.some((other) => other.row !== row &&
                ...
                lastEndTime = Math.max(lastEndTime, overlay.from + overlay.durationInFrames);
            }
        ...
        sortedOverlays.forEach((overlay) => {
            if (overlay.row < visibleRows) {
                rowEndTimes[overlay.row] = Math.max(rowEndTimes[overlay.row], overlay.from + overlay.durationInFrames);
            }
        });
        const earliestRow = rowEndTimes.reduce((minRow, endTime, index) => endTime < rowEndTimes[minRow] ? index : minRow, 0);
        return {
     ...
     * @returns Object containing the starting position (from) and row number
     */
    const addAtPlayhead = (currentFrame, existingOverlays, placement = 'top') => {
        if (placement === 'bottom') {
            // Find the highest row number currently in use
            const maxRow = existingOverlays.reduce((max, overlay) => Math.max(max, overlay.row), -1);
