 ...
 */
const findThumbnailFromOverlays = (overlays) => {
    // First try to find a video overlay
    const videoOverlay = overlays.find((overlay) => overlay.type === OverlayType.VIDEO);
    if (videoOverlay && "content" in videoOverlay) {
        return videoOverlay.content;
    }
    // Then try to find an image overlay
    const imageOverlay = overlays.find((overlay) => overlay.type === OverlayType.IMAGE);
 ...
 */
export const createStaticTemplateAdaptor = (displayName = "Default Templates") => ({
    ...
    getTemplates: async (params = {}) => {
        try {
            ...
            console.log("Default template adaptor getTemplates called with params:", params);
            // Import all template files dynamically
            const loadedTemplates = defaultTemplates.map((templateModule) => {
                const templateData = templateModule;
                // Convert overlays with proper type handling
                const processedOverlays = templateData.overlays.map((overlay) => {
                    console.log("Processing overlay:", overlay);
                    const overlayType = overlay.type.toUpperCase();
                ...
                const thumbnail = templateData.thumbnail ||
                    findThumbnailFromOverlays(processedOverlays);
                if (thumbnail) {
                    optionalProps.thumbnail = thumbnail;
                }
                const finalTemplate = Object.assign({}, baseTemplate, optionalProps);
            ...
            if (searchQuery && searchQuery.trim()) {
                ...
                filteredTemplates = loadedTemplates.filter((template) => template.name.toLowerCase().includes(query) ||
 ...
 */
export const defaultTemplateAdaptor = createStaticTemplateAdaptor("Default Templates");
/**
 * Helper function to get default template adaptors
 * This provides a consistent way to include default template content
 */
export const getDefaultTemplateAdaptors = () => {
