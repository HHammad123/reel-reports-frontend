...
import { TimelineTrack, TimelineGhostMarker, TimelineMarker, TimelineGuidelines } from './';
 ...
 */
export const TimelineContent = ({ tracks, viewportDuration, currentFrame, fps, zoomScale, onFrameChange, onItemSelect, onDeleteItems, onDuplicateItems, onSplitItems, selectedItemIds = [], onSelectedItemsChange, onItemMove, onItemResize, onNewItemDrop, timelineRef, ghostMarkerPosition, isContextMenuOpen, onMouseMove, onMouseLeave, onInsertTrackAt, onInsertMultipleTracksAt, onCreateTracksWithItems, showTimelineGuidelines = true, onContextMenuOpenChange, splittingEnabled = false, hideItemsOnDrag = false, }) => {
    const currentTime = currentFrame / fps;
    // Get state from timeline store
    const { draggedItem, ghostElement, floatingGhost, isValidDrop, isDragging: isDraggingStore, isPlayheadDragging, setTimelineRef, setIsPlayheadDragging, insertionIndex, setInsertionIndex, currentDragPosition, } = useTimelineStore();
    ...
    const { handleNewItemDragOver, handleNewItemDragEnd, handleNewItemDragLeave, handleNewItemDrop, clearNewItemDragState, } = useNewItemDrag({
        timelineRef,
        totalDuration: viewportDuration,
        tracks,
        onNewItemDrop,
    ...
    const { isMarqueeSelecting, marqueeStartPoint, marqueeEndPoint, handleTimelineMouseDown: marqueeHandleTimelineMouseDown, handleMarqueeMouseMove, handleMarqueeMouseUp, } = useMarqueeSelection({
        timelineRef,
        tracks,
        totalDuration: viewportDuration,
        selectedItemIds,
    ...
    useEffect(() => {
        ...
        const handleGlobalMouseMove = (e) => {
            ...
            if (!timelineRect)
                return;
            const isOutsideTimeline = (e.clientX < timelineRect.left ||
        ...
        const handleGlobalTouchMove = (e) => {
            ...
            if (!timelineRect)
                return;
            const isOutsideTimeline = (touch.clientX < timelineRect.left ||
    ...
    const enhancedMouseMove = useCallback((e) => {
    ...
    const enhancedTouchMove = useCallback((e) => {
    ...
    useEffect(() => {
        ...
        if (!markersWrapper || !tracksScrollContainer || !markersOverlay)
            return;
        const syncScroll = () => {
            const scrollLeft = markersWrapper.scrollLeft || tracksScrollContainer.scrollLeft;
            markersOverlay.style.transform = `translateX(-${scrollLeft}px)`;
        };
        // Initial sync
        syncScroll();
    ...
    return (_jsxs("div", { className: "flex flex-col h-full overflow-hidden relative", style: {
            ...
            '--ghost-marker-visible': '0',
                    ...
                    overflow: 'scroll' /* Always show scrollbar to keep same height and width */
                        ...
                        minHeight: 'fit-content',
                                ...
                                return (_jsx(TimelineTrack, { track: track, trackIndex: index, trackCount: tracks.length, totalDuration: viewportDuration, onItemSelect: onItemSelect, onDeleteItems: onDeleteItems, onDuplicateItems: onDuplicateItems, onSplitItems: onSplitItems, selectedItemIds: selectedItemIds, onSelectedItemsChange: onSelectedItemsChange, onItemMove: onItemMove, onDragStart: handleDragStart, zoomScale: zoomScale, isDragging: isDragging, draggedItemId: draggedItem === null || draggedItem === void 0 ? void 0 : draggedItem.id, ghostElements: trackGhostElements, isValidDrop: isValidDrop, onContextMenuOpenChange: onContextMenuOpenChange, splittingEnabled: splittingEnabled, hideItemsOnDrag: hideItemsOnDrag, currentFrame: currentFrame, fps: fps }, track.id));
                            }) }), showTimelineGuidelines && (_jsx(TimelineGuidelines, { tracks: tracks, totalDuration: viewportDuration, isDragging: isDragging, draggedItemId: draggedItem === null || draggedItem === void 0 ? void 0 : draggedItem.id, currentDragPosition: currentDragPosition })), _jsx(TimelineInsertionLine, { insertionIndex: insertionIndex, trackCount: tracks.length }), _jsx(TimelineMarqueeSelection, { isMarqueeSelecting: isMarqueeSelecting, marqueeStartPoint: marqueeStartPoint, marqueeEndPoint: marqueeEndPoint })] }) }), _jsx("div", { className: "absolute inset-0 pointer-events-none overflow-hidden", style: { zIndex: 45 }, children: _jsxs("div", { className: "timeline-markers-overlay-content h-full relative", style: {
                        ...getTimelineContentStyles(zoomScale),
                    }, children: [_jsx(TimelineMarker, { currentFrame: currentFrame, totalDurationInFrames: Math.ceil(viewportDuration * fps), zoomScale: zoomScale }), _jsx(TimelineGhostMarker, { position: ghostMarkerPosition, isDragging: isDragging || isPlayheadDragging, isContextMenuOpen: isContextMenuOpen, isScrubbing: false, isSplittingEnabled: splittingEnabled, totalDuration: viewportDuration, currentTime: currentTime, zoomScale: zoomScale })] }) }), floatingGhost && createPortal(_jsx(TimelineGhostElement, { ghostElement: {
                    left: 0, // Not used in floating mode
                    width: floatingGhost.width,
                    top: 0, // Not used in floating mode
                }, rowIndex: 0, trackCount: tracks.length, isValidDrop: floatingGhost.isValid, isFloating: true, floatingPosition: floatingGhost.position, itemData: floatingGhost.itemData }), document.body)] }));
};
