 ...
 */
export const TimelineGuidelines = ({ tracks, totalDuration, isDragging, draggedItemId, currentDragPosition, }) => {
    ...
    const guidelines = useMemo(() => {
        ...
        tracks.forEach((track, trackIndex) => {
            ...
            if (trackIndex === currentDragPosition.trackIndex)
                return;
    ...
    return (_jsx("div", { className: "absolute inset-0 pointer-events-none", style: { zIndex: 55 }, children: guidelines.map((guideline, index) => {
            const leftPercentage = (guideline.position / totalDuration) * 100;
            return (_jsx("div", { className: "absolute transition-opacity duration-150 border-l border-dashed border-(--timeline-ghost)", style: {
                    left: `${leftPercentage}%`,
                    top: 0, // Start from top since markers are in a separate container
                    bottom: 0,
                    width: '1px',
                    transform: 'translateX(-0.5px)', // Center the line
                } }, `guideline-${guideline.position}-${index}`));
        }) }));
};
