...
const formatTime = (timeInSeconds, totalDuration) => {
...
const calculateOptimalInterval = (totalDuration, zoomScale) => {
...
export const TimelineMarkers = ({ totalDuration, onTimeClick, onDragStateChange, zoomScale = 1 }) => {
    const handleMouseDown = useCallback((e) => {
        ...
        const handleMouseMove = (e) => {
            ...
            if (deltaX > 3 || deltaY > 3) {
                ...
                const container = document.querySelector('.timeline-markers-container');
                if (container) {
                    const rect = container.getBoundingClientRect();
                    const x = e.clientX - rect.left;
                    const clickPosition = Math.max(0, Math.min(1, x / rect.width));
                    const timeInSeconds = clickPosition * totalDuration;
                    onTimeClick === null || onTimeClick === void 0 ? void 0 : onTimeClick(Math.max(0, Math.min(totalDuration, timeInSeconds)));
                }
            }
        };
    ...
    const handleTouchStart = useCallback((e) => {
        ...
        if ('vibrate' in navigator && ((_a = navigator.userActivation) === null || _a === void 0 ? void 0 : _a.hasBeenActive)) {
            try {
    ...
    const generateMarkers = () => {
        ...
        for (let time = 0; time <= totalDuration; time += minorInterval) {
            ...
            if (!isMajor) {
                markers.push(_jsx("div", { className: "absolute", style: {
                        left: `${positionPercentage}%`,
                        transform: 'translateX(-50%)',
                    }, children: _jsx("div", { className: "w-px bg-gray-400 dark:bg-gray-400 h-1.5 opacity-60" }) }, `minor-${time}`));
            }
        }
        // Generate major ticks with labels
        for (let time = 0; time <= totalDuration; time += interval) {
            const positionPercentage = (time / totalDuration) * 100;
            markers.push(_jsxs("div", { className: "absolute flex flex-col items-center", style: {
                    left: `${positionPercentage}%`,
                    transform: 'translateX(-50%)',
                }, children: [_jsx("div", { className: "w-px bg-gray-400 dark:bg-gray-400 h-3" }), _jsx("span", { className: "text-xs mt-1 select-none whitespace-nowrap text-text-secondary font-extralight", children: formatTime(time, totalDuration) })] }, `major-${time}`));
        }
        return markers;
    };
    return (_jsx("div", { className: "relative bg-background border-b border-border cursor-pointer w-full timeline-markers-container", style: {
            height: `${TIMELINE_CONSTANTS.MARKERS_HEIGHT}px`,
            minWidth: zoomScale >= 1.0 ? "100%" : "auto"
        }, onMouseDown: handleMouseDown, onTouchStart: handleTouchStart, children: generateMarkers() }));
};
