...
export const useMarqueeSelection = ({ timelineRef, tracks, totalDuration, selectedItemIds, onSelectedItemsChange, isDragging = false, isContextMenuOpen = false, }) => {
    const [isMarqueeSelecting, setIsMarqueeSelecting] = useState(false);
    const [marqueeStartPoint, setMarqueeStartPoint] = useState(null);
    const [marqueeEndPoint, setMarqueeEndPoint] = useState(null);
    ...
    const handleMarqueeMouseMove = useCallback((e) => {
        ...
        if (isMarqueeSelecting &&
            ...
            marqueeThrottleRef.current = requestAnimationFrame(() => {
                var _a, _b;
                const rect = (_a = timelineRef.current) === null || _a === void 0 ? void 0 : _a.getBoundingClientRect();
                if (!rect)
                    return;
                ...
                const containerRect = (_b = timelineRef.current) === null || _b === void 0 ? void 0 : _b.getBoundingClientRect();
                if (!containerRect)
                    return;
                ...
                if (newlySelectedIds.size !== currentSelectedIds.size ||
