...
export const useTimelineComposition = ({ tracks, totalDuration, currentFrame, fps, zoomScale }) => {
    ...
    const viewportDuration = useMemo(() => calculateViewportDuration(compositionDuration, zoomScale), [compositionDuration, zoomScale]);
    // Convert current frame to time
    const currentTime = useMemo(() => frameToTime(currentFrame, fps), [currentFrame, fps]);
    return {
        compositionDuration,
        viewportDuration,
        currentTime,
