...
export const useTimelineDragAndDrop = ({ totalDuration, tracks, onItemMove, onItemResize, timelineRef, onInsertTrackAt, onCreateTracksWithItems, selectedItemIds = [], }) => {
    const { setDraggedItem, setGhostElement, setFloatingGhost, setIsValidDrop, setDragInfo, getDragInfo, resetDragState, setIsDragging, setInsertionIndex, setMagneticPreview, setCurrentDragPosition, } = useTimelineStore();
    const lastUpdateTime = useRef(0);
    // Enhanced snapping function that checks for both grid and edge snapping
    const snapToGridAndEdges = useCallback((value, trackIndex, excludeIds = []) => {
        ...
        if (trackIndex > 0)
            tracksToCheck.push(trackIndex - 1);
    ...
    const validateDropPosition = useCallback((targetStart, targetDuration, targetTrackIndex, excludeIds = [], action) => {
            ...
            if (dragInfo && dragInfo.selectedItemsSnapshot.length > 0) {
                ...
                if (primaryItem && (primaryItem.type === 'video' || primaryItem.type === 'audio') && primaryItem.mediaSrcDuration !== undefined) {
                    ...
                    if (action === "resize-start") {
                        ...
                        if (startTimeDelta < 0 && (primaryItem.mediaStart || 0) + startTimeDelta < 0) {
                            return {
                                isValid: false,
        ...
        if (targetTrack.magnetic) {
            ...
            if (action === "move") {
                ...
                const preview = calculateMagneticInsertionPreview(availableItems, targetDuration, targetStart);
                return {
                    isValid: true,
        ...
        if (action === "resize-start" || action === "resize-end") {
            ...
            if (resizedItemId) {
                const result = pushItemsDuringResize(targetTrack.items, resizedItemId, targetStart, targetStart + targetDuration);
                // Use the actual achievable position for validation
                return {
                    isValid: true,
                    magneticStart: result.actualStart, // Return the constrained start position
                    magneticPreview: result.items.map((item) => ({
                        id: item.id,
                        start: item.start,
                        end: item.end,
            ...
            const overlappingItems = targetTrack.items.filter((item) => !excludeIds.includes(item.id) &&
    ...
    const calculateGhostPosition = useCallback((startTime, duration, trackIndex) => {
    ...
    const handleDragStart = useCallback((item, clientX, clientY, action, selectedItemIds = [] // Add selectedItemIds parameter
    ) => {
    ...
    const handleDrag = useCallback((clientX, clientY) => {
        var _a, _b;
        const dragInfo = getDragInfo();
        if (!dragInfo || !timelineRef.current)
            return;
        ...
        switch (dragInfo.action) {
            case "move":
                newStart = snapToGridAndEdges(dragInfo.startPosition + deltaTime, targetTrackIndex, excludeIds);
            ...
            case "resize-end":
                ...
                const snappedEndPosition = snapToGridAndEdges(dragInfo.startPosition + rawNewDuration, dragInfo.startRow, excludeIds);
        ...
        const validation = validateDropPosition(newStart, newDuration, newTrackIndex, excludeIds, dragInfo.action);
        ...
        if (isMultiDrag) {
            ...
            for (const snapshot of dragInfo.selectedItemsSnapshot) {
                ...
                const itemValidation = validateDropPosition(itemNewStart, itemNewDuration, itemNewTrackIndex, excludeIds, dragInfo.action);
        ...
        const contentScrollTop = timelineRef.current.scrollTop || 0;
        ...
        if (distanceToBoundary < SHOW_THRESHOLD && dragInfo.action === "move") {
            ...
            setFloatingGhost({
                ...
                itemData: {
                    type: (_a = dragInfo.selectedItemsSnapshot[0]) === null || _a === void 0 ? void 0 : _a.type,
                    label: isMultiDrag
                        ? `${dragInfo.selectedItemsSnapshot.length} items`
                        : (_b = dragInfo.selectedItemsSnapshot[0]) === null || _b === void 0 ? void 0 : _b.label,
                },
            });
    ...
    const handleDragEnd = useCallback(() => {
        ...
        if ((!ghostElements || ghostElements.length === 0) && insertionIdx !== null) {
            if (isMultiDrag && onCreateTracksWithItems) {
                ...
                const primarySnapshot = dragInfo.selectedItemsSnapshot.find(s => s.id === dragInfo.id);
                if (primarySnapshot) {
                    const primaryFinalStart = snapToGrid((_a = dragInfo.currentStart) !== null && _a !== void 0 ? _a : dragInfo.startPosition);
                    ...
                    const itemsByOriginalTrack = new Map();
                    for (const snapshot of dragInfo.selectedItemsSnapshot) {
                ...
                const finalStart = snapToGrid((_b = dragInfo.currentStart) !== null && _b !== void 0 ? _b : dragInfo.startPosition);
                const finalDuration = snapToGrid((_c = dragInfo.currentDuration) !== null && _c !== void 0 ? _c : dragInfo.startDuration);
        ...
        if (isMultiDrag && dragInfo.action === "move" && onItemMove) {
            ...
            else if ((dragInfo.action === "resize-start" || dragInfo.action === "resize-end") &&
                onItemResize) {
    ...
    const wrappedHandleDragStart = useCallback((item, clientX, clientY, action) => {
