...
export const useTimelineTracks = ({ initialTracks, autoRemoveEmptyTracks, onTracksChange, selectedItemIds = [], onSelectedItemsChange }) => {
    const [tracks, setTracks] = useState(initialTracks);
    const [isAutoRemoveEnabled, setIsAutoRemoveEnabled] = useState(autoRemoveEmptyTracks);
    ...
    const removeEmptyTracks = useCallback((tracks, shouldRemove = isAutoRemoveEnabled) => {
        if (!shouldRemove)
            return tracks;
    ...
    const addNewItem = useCallback((itemData, currentFrame, fps) => {
        const currentTime = frameToTime(currentFrame, fps);
        try {
            // Create new item with intelligent positioning
            const newItemTemplate = createNewItemWithIntelligentPositioning(tracks, itemData, currentTime);
            // Generate unique ID
            ...
            setTracks(prevTracks => {
                const newTracks = prevTracks.map(track => {
                    if (track.id === newItem.trackId) {
                        ...
                        if (track.magnetic) {
                            ...
                            const preview = calculateMagneticInsertionPreview(track.items, newItem.end - newItem.start, newItem.start);
                ...
                onTracksChange === null || onTracksChange === void 0 ? void 0 : onTracksChange(updatedTracks);
    ...
    const handleItemMove = useCallback((itemId, newStart, newEnd, newTrackId) => {
        setTracks(prevTracks => {
            // Find the source track of the moved item for magnetic gap closing
            const sourceTrack = prevTracks.find(track => track.items.some(item => item.id === itemId));
            const newTracks = prevTracks.map(track => ({
                ...track,
            ...
            if (targetTrackIndex !== -1) {
                ...
                if (movedItem) {
                    ...
                    if (targetTrack.magnetic) {
                        ...
                        const preview = calculateMagneticInsertionPreview(itemsForInsertion, itemDuration, newStart // Use the intended position
                        );
    ...
    const handleItemResize = useCallback((itemId, newStart, newEnd) => {
        setTracks(prevTracks => {
    ...
    const handleItemsDelete = useCallback((itemIds) => {
        setTracks(prevTracks => {
            const newTracks = prevTracks.map(track => {
    ...
    const handleInsertTrackAt = useCallback((index) => {
        let newId = '';
        setTracks(prev => {
            const newTrack = {
                id: `track-${Date.now()}-${Math.random().toString(36).slice(2, 7)}`,
            ...
            onTracksChange === null || onTracksChange === void 0 ? void 0 : onTracksChange(next);
            return next;
        });
        return newId;
    }, [onTracksChange]);
    // Insert multiple new empty tracks starting at a given index and return their IDs
    const handleInsertMultipleTracksAt = useCallback((index, count) => {
        const newIds = [];
        setTracks(prev => {
            const next = [...prev];
            const clampedIndex = Math.max(0, Math.min(next.length, index));
    ...
    const handleCreateTracksWithItems = useCallback((index, trackItems) => {
        setTracks(prev => {
            // First, remove all the items from their current tracks
            const next = prev.map(track => ({
                ...track,
                items: track.items.filter(item => !trackItems.some(trackGroup => trackGroup.items.some(itemInfo => itemInfo.itemId === item.id)))
            }));
            const clampedIndex = Math.max(0, Math.min(next.length, index));
    ...
    // Track reordering
    const handleTrackReorder = useCallback((fromIndex, toIndex) => {
        setTracks(prev => {
            if (fromIndex < 0 ||
                toIndex < 0 ||
    ...
    const clearSelectionForDeletedTrack = useCallback((trackToDelete) => {
        ...
        const selectedItemsInDeletedTrack = selectedItemIds.filter(itemId => itemsInDeletedTrack.includes(itemId));
        // If any selected items are in the deleted track, clear them from selection
        if (selectedItemsInDeletedTrack.length > 0) {
            const remainingSelectedItems = selectedItemIds.filter(itemId => !itemsInDeletedTrack.includes(itemId));
    ...
    const handleToggleMagnetic = useCallback((trackId) => {
        setTracks(prev => {
