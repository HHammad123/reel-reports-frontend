 ...
 */
export const findGapsInTrack = (trackItems) => {
 ...
 */
export const closeGapsInTrack = (trackItems) => {
    if (trackItems.length === 0)
        return [];
    // Sort items by start time
    const sortedItems = [...trackItems].sort((a, b) => a.start - b.start);
 ...
 */
export const calculateMagneticDropPosition = (trackItems) => {
    if (trackItems.length === 0)
        return 0;
 ...
 */
export const calculateMagneticInsertionPreview = (trackItems, itemDuration, intendedStart = 0) => {
    if (trackItems.length === 0) {
        return {
            insertionIndex: 0,
            insertionStart: 0,
 ...
 */
export const pushItemsDuringResize = (trackItems, resizedItemId, newStart, newEnd) => {
    if (trackItems.length === 0) {
        return {
            items: [],
    ...
    if (expandingLeft) {
        ...
        const blockingItems = trackItems.filter(item => item.id !== resizedItemId &&
            item.end > newStart &&
    ...
    if (expandingRight) {
        ...
        for (let i = 0; i < updatedItems.length; i++) {
            ...
            if (currentItem.id === resizedItemId)
                continue;
 ...
 */
export const calculatePushingResizePreview = (trackItems, resizedItemId, newStart, newEnd) => {
 ...
 */
export const canFitAtPosition = (trackItems, startTime, duration) => {
    const endTime = startTime + duration;
    return !trackItems.some(item => startTime < item.end && endTime > item.start);
};
/**
 ...
 */
export const findBestPositionForNewItem = (tracks, duration, currentTime, preferredTrackId, preferredStartTime) => {
    ...
    const leastPopulated = trackEndTimes.reduce((min, current) => current.endTime < min.endTime ? current : min);
 ...
 */
export const createNewItemWithIntelligentPositioning = (tracks, itemData, currentTime) => {
    var _a, _b;
    // Default duration if not provided
    const duration = (_a = itemData.duration) !== null && _a !== void 0 ? _a : 3;
    // Find the best position
    const { trackId, startTime } = findBestPositionForNewItem(tracks, duration, currentTime, itemData.preferredTrackId, itemData.preferredStartTime);
    return {
        trackId,
        start: startTime,
        end: startTime + duration,
        label: (_b = itemData.label) !== null && _b !== void 0 ? _b : `New ${itemData.type}`,
        type: itemData.type,
        color: '#6b7280',
        data: itemData.data
    };
