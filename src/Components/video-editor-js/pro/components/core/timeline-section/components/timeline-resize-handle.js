 ...
 */
export const TimelineResizeHandle = ({ onMouseDown, onTouchStart, isResizing }) => {
    return (_jsx("div", { onMouseDown: onMouseDown, onTouchStart: onTouchStart, className: `
        h-1 bg-border hover:bg-primary transition-colors cursor-ns-resize
        flex items-center justify-center group relative
        ${isResizing ? 'bg-primary' : ''}
      `, style: {
            touchAction: 'none',
        }, children: _jsx("div", { className: "absolute inset-x-0 top-0 h-1 " }) }));
};
