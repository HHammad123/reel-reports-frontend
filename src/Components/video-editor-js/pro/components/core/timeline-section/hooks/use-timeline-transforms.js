...
export const useTimelineTransforms = () => {
     ...
     */
    const transformOverlaysToTracks = React.useCallback((overlays) => {
     ...
     */
    const transformTracksToOverlays = React.useCallback((tracks) => {
        const overlays = [];
        tracks.forEach((track, trackIndex) => {
            track.items.forEach(item => {
                if (item.data && typeof item.data === 'object') {
                    // Use the original overlay data if available
                    const originalOverlay = item.data;
                    ...
                    if (originalOverlay.type === OverlayType.VIDEO && item.mediaStart !== undefined) {
                        ...
                        updatedOverlay.videoStartTime = item.mediaStart;
                    }
                        ...
                        updatedOverlay.startFromSound = Math.round(item.mediaStart * FPS);
                    }
 ...
 */
const mapOverlayTypeToTimelineType = (type) => {
    switch (type) {
        case OverlayType.TEXT:
            return 'text';
        case OverlayType.IMAGE:
