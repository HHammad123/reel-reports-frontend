...
export function LocalMediaGallery({ onSelectMedia, sessionMedia = [], onClearAll }) {
    ...
    const normalizedSessionMedia = useMemo(() => {
        ...
        return base.map((m, idx) => {
            ...
            return ({
                id: m.id || `session-${idx}`,
                name: m.title || m.name || `Session Video ${idx + 1}`,
                path,
                type: m.type || "video",
    ...
    const renderPreviewContent = () => {
        ...
        if (!displayPath) {
            return (_jsx("div", { className: "text-sm text-secondary", children: "No source available for preview." }));
        }
        switch (selectedFile.type) {
            case "image":
                return (_jsx("div", { className: "relative bg-card rounded-lg p-2 flex justify-center", children: _jsx("img", { src: displayPath, alt: selectedFile.name, className: `${commonClasses} object-contain` }) }));
            case "video":
                return (_jsx("div", { className: "relative bg-card rounded-lg p-2", children: _jsx("video", { src: displayPath, controls: true, className: commonClasses, controlsList: "nodownload", playsInline: true, onError: (e) => console.error('[LocalMediaGallery] Video preview error', e) }) }));
            case "audio":
                return (_jsxs("div", { className: "flex flex-col items-center space-y-3 p-4 bg-card rounded-lg", children: [_jsx("div", { className: "w-12 h-12 bg-primary/10 rounded-full flex items-center justify-center", children: _jsx(Music, { className: "w-6 h-6" }) }), _jsx("audio", { src: selectedFile.path.startsWith("http")
                                ? selectedFile.path
            ...
            default:
                return (_jsx("div", { className: "text-sm text-secondary", children: "Unsupported file type" }));
        }
    };
    ...
    const renderMediaItem = useCallback((file) => {
        ...
        return (_jsxs("div", { className: "relative group/item border rounded-md overflow-hidden cursor-pointer \n          hover:border-primary transition-all \n          bg-card shadow-sm hover:shadow-md", onClick: () => handleMediaSelect(file), draggable: true, onDragStart: handleDragStart(file), onDragEnd: handleDragEnd, children: [_jsxs("div", { className: "aspect-video relative", children: [file.type === "image" && (_jsx("img", { src: resolvedThumb, alt: file.name, className: "absolute inset-0 w-full h-full object-cover bg-card", draggable: false })), file.type === "video" && (_jsxs(_Fragment, { children: [_jsx("img", { src: resolvedThumb, alt: file.name, className: "absolute inset-0 w-full h-full object-cover bg-card", draggable: false }), _jsx("div", { className: "absolute bottom-1.5 right-1.5 bg-black/75 text-foreground text-xs px-1.5 py-0.5 rounded-md", children: formatDuration(file.duration) })] })), file.type === "audio" && (_jsx("div", { className: "w-full h-full flex items-center justify-center bg-card", children: _jsx(Music, { className: "w-10 h-10" }) }))] }), _jsxs("div", { className: "p-2.5", children: [_jsx("p", { className: "text-sm font-extralight truncate text-foreground", children: file.name }), _jsx("p", { className: "text-xs text-muted-foreground mt-0.5", children: formatBytes(file.size) })] }), _jsx("button", { className: "absolute top-2 right-2 bg-destructive\n            text-destructive-foreground p-1.5 rounded-full opacity-0 group-hover/item:opacity-100 transition-all duration-200 \n            shadow-sm hover:shadow-md transform hover:scale-105", onClick: (e) => {
                        e.stopPropagation();
                        if (!file._session) {
    ...
    return (_jsxs("div", { className: "h-full flex flex-col", children: [_jsxs("div", { className: "flex justify-between items-center mb-4", children: [_jsx("h2", { className: "text-sm font-extralight", children: "Saved Uploads" }), _jsxs("div", { className: "flex items-center gap-2", children: [onClearAll && (_jsx(Button, { variant: "outline", size: "sm", className: "gap-1 text-destructive hover:text-destructive", onClick: onClearAll, children: [_jsx(X, { className: "w-4 h-4" }), "Clear All"] })), _jsxs(Button, { variant: "default", size: "sm", className: "gap-1", onClick: handleUploadClick, disabled: isLoading, children: [isLoading ? (_jsx(Loader2, { className: "w-4 h-4 animate-spin" })) : (_jsx(Upload, { className: "w-4 h-4" })), "Upload"] }), _jsx("input", { ref: fileInputRef, id: "file-upload", type: "file", className: "hidden", onChange: handleFileUpload, accept: "image/*,video/*,audio/*", disabled: isLoading })] })] }), uploadError && (_jsx("div", { className: "bg-destructive/10 border border-destructive/20 text-destructive px-4 py-2 rounded mb-4", children: uploadError })), _jsxs("div", { className: "flex-1 flex flex-col", children: [_jsx(UnifiedTabs, { sourceResults: sourceResults, activeTab: activeTab, onTabChange: setActiveTab, className: "mb-4" }), _jsx("div", { className: "flex-1 overflow-y-auto", children: isLoading ? (_jsxs("div", { className: "h-full flex flex-col items-center justify-center text-center space-y-4 text-sm text-muted-foreground", children: [_jsx(Loader2, { className: "w-5 h-5 animate-spin" }), _jsx("p", { children: "Loading media files..." })] })) : filteredMedia.length === 0 ? (_jsxs("div", { className: "h-full flex flex-col font-extralight items-center justify-center py-8 text-muted-foreground text-center", children: [_jsx(Upload, { className: "h-8 w-8 mb-2" }), _jsx("p", { className: "text-sm text-center", children: "No media files" }), _jsx("p", { className: "text-xs text-center mt-1", children: "Upload your first media file to get started" }), _jsx(Button, { variant: "default", size: "sm", onClick: handleUploadClick, className: "text-xs mt-3", children: "Upload Media" })] })) : (_jsx("div", { className: "grid grid-cols-2 sm:grid-cols-2 gap-2", children: filteredMedia.map(renderMediaItem) })) })] }), _jsx(Dialog, { open: previewOpen, onOpenChange: setPreviewOpen, children: _jsxs(DialogContent, { className: "max-w-2xl", children: [_jsxs(DialogHeader, { children: [_jsx(DialogTitle, { children: selectedFile === null || selectedFile === void 0 ? void 0 : selectedFile.name }), _jsxs(DialogDescription, { children: [selectedFile === null || selectedFile === void 0 ? void 0 : selectedFile.type, " \u2022 ", formatBytes(selectedFile === null || selectedFile === void 0 ? void 0 : selectedFile.size)] })] }), _jsx("div", { className: "flex justify-center", children: renderPreviewContent() }), _jsx("div", { className: "flex justify-end mt-4", children: _jsx(Button, { variant: "default", size: "sm", onClick: handleAddToTimeline, children: "Add to Timeline" }) })] }) })] }));
}
