...
export const MediaGrid = ({
  ...
  // Group items by session name for "All" tab (for images from generate-media API and videos from generate-base-video API)
  const groupedBySession =
    activeTab === "all" &&
    (mediaType === "images" || mediaType === "videos") &&
    items.some((item) => item._sessionName);
  ...
  if (items.length > 0) {
    // Render grouped by session if applicable
    if (groupedBySession && sessionGroups) {
      return _jsx("div", {
        className: "flex flex-col gap-6",
        children: sessionGroups.map(([sessionName, sessionItems]) =>
          _jsxs(
            "div",
            {
              ...
              children: [
                ...
                _jsx("div", {
                  className: "columns-2 sm:columns-2 gap-3 space-y-3",
                  children: sessionItems.map((item) => {
                    const itemKey = getItemKey(item);
                    const isItemLoading =
                      isDurationLoading && loadingItemKey === itemKey;
                    ...
                    const isSessionVideo =
                      item._isSessionVideo || item._sessionVideo;
                    ...
                    const shouldShowVideo =
                      ...
                      videoUrl &&
                      (isSessionVideo ||
                        (!hasRealThumbnail && !hasRealPoster));
    ...
    // Regular grid display (no grouping)
    return _jsx("div", {
      className: "columns-2 sm:columns-2 gap-3 space-y-3",
      children: items.map((item) => {
        ...
        const shouldShowVideo =
          isVideo &&
          videoUrl &&
          (isSessionVideo || (!hasRealThumbnail && !hasRealPoster));
