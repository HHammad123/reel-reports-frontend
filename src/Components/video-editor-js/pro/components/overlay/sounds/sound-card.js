 ...
 * @component
 */
const SoundCard = ({ sound, playingTrack, onTogglePlay, onAddToTimeline, showSourceBadge = false, enableTimelineDrag = false, }) => {
    ...
    return (_jsxs("div", { onClick: () => onAddToTimeline(sound), draggable: enableTimelineDrag, onDragStart: handleDragStart, onDragEnd: handleDragEnd, className: "group flex items-center gap-3 p-3 bg-card rounded-md \n        border border-sidebar-border hover:bg-accent\n         duration-150 cursor-pointer relative shadow-sm border-base-300", children: [_jsx(Button, { variant: "ghost", size: "sm", onClick: (e) => {
                    e.stopPropagation();
                    onTogglePlay(sound.id);
                }, className: "h-8 w-8 rounded-full shrink-0 text-foreground", children: playingTrack === sound.id ? (_jsx(Pause, { className: "h-4 w-4" })) : (_jsx(Play, { className: "h-4 w-4" })) }), _jsxs("div", { className: "min-w-0 flex-1 pr-2", children: [_jsx("p", { className: "text-sm font-extralight text-foreground truncate mb-1", children: sound.title }), _jsxs("p", { className: "text-xs font-extralight text-muted-foreground truncate", children: [sound.artist, " \u2022 ", sound.duration, "s"] })] }), showSourceBadge && (_jsx("span", { className: "absolute font-extralight top-2 right-2 text-[10px] px-1 py-0.5 bg-muted text-muted-foreground rounded text-center leading-none", children: sound._sourceDisplayName }))] }));
};
