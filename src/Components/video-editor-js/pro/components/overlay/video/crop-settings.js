 ...
 * - Reset crop button
 */
export const CropSettings = ({ localOverlay, handleStyleChange, }) => {
    var _a, _b;
    const isCropEnabled = (_b = (_a = localOverlay === null || localOverlay === void 0 ? void 0 : localOverlay.styles) === null || _a === void 0 ? void 0 : _a.cropEnabled) !== null && _b !== void 0 ? _b : false;
    /**
     ...
     */
    const handleApplyAspectRatio = (targetRatio) => {
        if (!localOverlay)
            return;
    ...
    const handleCropToggle = (enabled) => {
        ...
        if (enabled) {
            ...
            if (((_a = localOverlay === null || localOverlay === void 0 ? void 0 : localOverlay.styles) === null || _a === void 0 ? void 0 : _a.cropX) === undefined) {
                updates.cropX = 0;
                updates.cropY = 0;
    ...
    return (_jsxs("div", { className: "space-y-4 rounded-md bg-card p-4 border", children: [_jsxs("div", { className: "flex items-center justify-between", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Crop, { className: "w-4 h-4 text-muted-foreground" }), _jsx("h3", { className: "text-xs font-extralight text-foreground", children: "Crop" })] }), _jsx(Switch, { checked: isCropEnabled, onCheckedChange: handleCropToggle })] }), isCropEnabled && (_jsxs(_Fragment, { children: [_jsx(Separator, {}), _jsxs("div", { className: "space-y-3", children: [_jsxs("div", { children: [_jsx("label", { className: "text-xs mb-2 block font-extralight text-foreground", children: "Aspect Ratios" }), _jsx("div", { className: "grid grid-cols-4 gap-2", children: ASPECT_RATIOS.map((aspectRatio) => (_jsxs("button", { onClick: () => handleApplyAspectRatio(aspectRatio.ratio), className: "flex flex-col items-center justify-center p-2 rounded-md border border-border bg-background hover:bg-accent hover:border-accent-foreground transition-colors group", title: `Apply ${aspectRatio.label} aspect ratio`, children: [_jsx("div", { className: "bg-muted/50 hover:bg-accent-foreground/10 transition-colors mb-1.5 border group-hover:border-accent-foreground/20 border-border", style: {
                                                        width: aspectRatio.ratio >= 1 ? '28px' : `${28 * aspectRatio.ratio}px`,
                                                        height: aspectRatio.ratio >= 1 ? `${28 / aspectRatio.ratio}px` : '28px',
                                                    } }), _jsx("span", { className: "text-[10px] text-muted-foreground group-hover:text-foreground", children: aspectRatio.label })] }, aspectRatio.value))) })] }), _jsx(Separator, {}), _jsxs("div", { className: "flex items-center justify-between ", children: [_jsx("span", { className: "text-xs text-muted-foreground font-extralight", children: "Use handles to fine-tune" }), _jsx("button", { onClick: handleClearCrop, className: "text-xs px-3 py-1.5 rounded-md transition-colors bg-secondary hover:bg-secondary/80 text-secondary-foreground", children: "Reset" })] })] })] }))] }));
};
