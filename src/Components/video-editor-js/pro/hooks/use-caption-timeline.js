 ...
 */
export const validateTiming = (startMs, endMs, captionIndex, captions) => {
    if (startMs >= endMs) {
...
export const useCaptionTimeline = ({ localOverlay, setLocalOverlay, currentMs, }) => {
    const containerRef = useRef(null);
    const activeCaptionRef = useRef(null);
    const lastScrolledCaptionIndex = useRef(-1);
    ...
    useEffect(() => {
        ...
        const activeIndex = localOverlay.captions.findIndex((caption) => relativeCurrentMs >= caption.startMs && relativeCurrentMs < caption.endMs);
    ...
    }, [currentMs, localOverlay === null || localOverlay === void 0 ? void 0 : localOverlay.captions, localOverlay === null || localOverlay === void 0 ? void 0 : localOverlay.from]);
    const getInputKey = (captionIndex, field) => `${captionIndex}-${field}`;
    const getInputValue = (captionIndex, field) => {
    ...
    const handleInputChange = (captionIndex, field, value) => {
        const key = getInputKey(captionIndex, field);
    ...
    const handleTimingChange = (captionIndex, field, timeString) => {
        if (!(localOverlay === null || localOverlay === void 0 ? void 0 : localOverlay.captions))
            return;
        ...
        // Validate timing
        const validation = validateTiming(updatedCaption.startMs, updatedCaption.endMs, captionIndex, newCaptions);
