...
export const useCaptions = () => {
     ...
     */
    const validateSRTStructure = useCallback((content) => {
        const errors = [];
     ...
     */
    const distributeWordTiming = useCallback((words, startMs, endMs) => {
        if (words.length === 0)
            return [];
     ...
     */
    const parseSRT = useCallback(async (content) => {
     ...
     */
    const generateFromText = useCallback(async ({ text, wordsPerMinute = 160, sentenceGapMs = 500, }) => {
        setState(prev => ({ ...prev, isProcessing: true, isError: false, error: null }));
        try {
            if (!text.trim()) {
            ...
            const captions = sentences.map((sentence) => {
                ...
                const wordTiming = distributeWordTiming(words, sentenceStartTime, sentenceStartTime + words.length * msPerWord);
                const caption = {
                    text: sentence,
     ...
     */
    const createCaptionOverlay = useCallback((captions) => {
