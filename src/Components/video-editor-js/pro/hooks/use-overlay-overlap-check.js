...
export const useOverlayOverlapCheck = () => {
     ...
     */
    const checkOverlap = useCallback((overlay, currentOverlays) => {
        // Get all other overlays in the same row (excluding self if it's an existing overlay)
        const overlaysInRow = currentOverlays.filter((o) => o.row === overlay.row && o.id !== overlay.id);
        ...
        return overlaysInRow.some((existingOverlay) => {
            ...
            if (existingOverlay.id === overlay.id)
                return false;
            ...
            return ((overlay.from >= existingOverlay.from - buffer &&
     ...
     */
    const checkAndAdjustOverlaps = useCallback((overlay, currentOverlays) => {
        ...
        sortedOverlays.forEach((existingOverlay) => {
            ...
            if ((existingOverlay.from >= overlay.from - gap &&
