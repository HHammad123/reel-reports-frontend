...
export class MediaBunnyThumbnailService {
    ...
    static async generateThumbnail(file, options = {}) {
        const { width = MediaBunnyThumbnailService.DEFAULT_WIDTH, height = MediaBunnyThumbnailService.DEFAULT_HEIGHT, timestampSeconds = MediaBunnyThumbnailService.DEFAULT_TIMESTAMP, quality = MediaBunnyThumbnailService.DEFAULT_QUALITY, } = options;
        try {
     ...
     */
    static async generateMultipleThumbnails(file, timestamps, options = {}) {
        const thumbnails = [];
     ...
     */
    static async canProcessFile(file) {
        try {
            const input = new Input({
                source: new BlobSource(file),
                formats: ALL_FORMATS,
...
MediaBunnyThumbnailService.DEFAULT_WIDTH = 320;
MediaBunnyThumbnailService.DEFAULT_HEIGHT = 180;
MediaBunnyThumbnailService.DEFAULT_QUALITY = 0.8;
MediaBunnyThumbnailService.DEFAULT_TIMESTAMP = 1; // 1 second into video
