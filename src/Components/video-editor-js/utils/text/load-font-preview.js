...
const globalFailedFonts = new Set();
/**
 ...
 */
export const makeFontPreviewName = (fontFamily) => {
 ...
 */
export const loadFontPreview = async (previewName, fontFamily) => {
    ...
    try {
        ...
        const response = await fetch(`https://fonts.googleapis.com/css?family=${previewName}:400&text=${encodeURIComponent(fontFamily)}`);
        if (!response.ok) {
...
export const useFontPreviewLoader = () => {
    ...
    const loadFontForPreview = useCallback(async (fontFamily, previewUrl) => {
    ...
    const isFontLoaded = useCallback((fontFamily) => {
